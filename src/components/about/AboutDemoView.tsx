...
export const AboutDemoView: React.FC = () => {
  ...
  const [expandedTool, setExpandedTool] = useState<string | null>('search_jobs');
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      ...
      <div className="space-y-4">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-semibold">
          ...
          <span>WebMCP Browser Standard Demonstration</span>
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-zinc-900">
          About TrueRemoteJobs
        </h1>
        <p className="text-base sm:text-lg text-zinc-600 leading-relaxed max-w-3xl">
          TrueRemoteJobs demonstrates how client-side web applications expose high-level structured capabilities to AI agents using the <strong>Web Model Context Protocol (WebMCP)</strong> standard. Rather than scraping HTML DOM elements or simulating mouse clicks, external agents invoke programmatic tools directly through <code className="text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded font-mono text-sm">document.modelContext</code>.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200/90 p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          <div className="p-3.5 rounded-xl bg-zinc-50 border border-zinc-200/70 space-y-1">
            ...
            <p className="text-zinc-500 text-[11px]">Fallback to navigator.modelContext</p>
          </div>
          <div className="p-3.5 rounded-xl bg-zinc-50 border border-zinc-200/70 space-y-1">
          ...
          <div className="p-3.5 rounded-xl bg-zinc-50 border border-zinc-200/70 space-y-1">
            ...
            <p className="text-zinc-500 text-[11px]">Deterministic & instant local search</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-xl font-bold text-zinc-900 flex items-center gap-2">
            ...
            Hackathon Judge Demonstration Flow
          </h2>
          <p className="text-sm text-zinc-500 mt-1">
            Try these example commands in your WebMCP-enabled AI agent to experience the full end-to-end workflow:
          </p>
        </div>
      ...
      {/* Exposed WebMCP Tools List with Schemas */}
      <div className="space-y-4">
        <div>
          <h2 className="text-xl font-bold text-zinc-900 flex items-center gap-2">
            <Code2 className="w-5 h-5 text-indigo-600" />
            Exposed WebMCP Tools & Schemas (7 Tools)
          </h2>
          <p className="text-sm text-zinc-500 mt-1">
            Inspect the structured schemas registered for external agent invocation:
          </p>
        </div>
          ...
          {ALL_WEBMCP_TOOLS.map((tool) => {
            ...
            return (
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 pt-1 border-t border-zinc-100 bg-zinc-50/50 space-y-3">
      ...
      <div className="p-5 rounded-2xl bg-amber-50/80 border border-amber-200 text-amber-900 space-y-2">
        ...
        <p className="text-xs text-amber-800 leading-relaxed">
          TrueRemoteJobs is a prototype application built exclusively for hackathon evaluation of the Web Model Context Protocol. All positions, companies (e.g., NovaStack, Orbit Labs, Mosaic AI), salaries, and application policies are generated fictional records. No real applications will be processed or submitted.
        </p>
      </div>
